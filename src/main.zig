...
fn init_targets() [TARGET_ROWS * TARGET_COLS]Target {
    ...
    while (row < TARGET_ROWS) : (row += 1) {
        ...
        while (col < TARGET_COLS) : (col += 1) {
            targets[row * TARGET_COLS + col] = Target{ .x = TARGET_GRID_X + (TARGET_WIDTH + TARGET_PADDING_X) * @as(f32, @floatFromInt(col)), .y = TARGET_GRID_Y + TARGET_PADDING_Y * @as(f32, @floatFromInt(row)) };
        }
    }
    return targets;
}
...
fn make_rect(x: f32, y: f32, w: f32, h: f32) c.SDL_Rect {
    return c.SDL_Rect{ .x = @as(i32, @intFromFloat(x)), .y = @as(i32, @intFromFloat(y)), .w = @as(i32, @intFromFloat(w)), .h = @as(i32, @intFromFloat(h)) };
}
